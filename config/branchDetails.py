branch_details = {
    "00": "B Pharmacy",
    "01": "Civil Engineering",
    "02": "Electrical & Electronics Engineering",
    "03": "Mechanical Engineering",
    "04": "Electronics & Communication Engineering",
    "05": "Computer Science & Engineering",
    "08": "Chemical Engineering",
    "10": "Electronics & Instrumentation Engineering",
    "11": "BioMedical Engineering",
    "12": "Information Technology",
    "13": "Electronics & Control Engineering",
    "19": "Electronics & Computer Engineering",
    "21": "Aeronautical Engineering",
    "22": "Instrumentation & Control Engineering",
    "23": "Bio Technology",
    "32": "CS & Business Systems",
    "33": "CS & IT",
    "34": "IT and Engineering",
    "35": "Artificial Intelligence",
    "36": "CS and Technology",
    "56": "Computer Engineering ( SE ) Technology",
    "62": "CSE ( Cyber Security ) Technology",
    "66": "CSE ( AI and ML )",
    "67": "CSE ( Datascience )",
    "69": "CSE ( IoT )",
    "70": "CSE ( Networks )",
    "72": "AI & Data Science",
    "73": "AI & ML",
    "74": "Computer Science and Design",
    "75": "Automation and Robotics",
}


def get_branch_name(rollNumber: str) -> str:
    """
    Returns the branch name for a given code.
    If the code doesn't exist, returns 'Unknown branch'.
    """
    return branch_details.get(rollNumber[6:8], "Unknown")
